...
public class WorldGuardCommands {
    ...
    @Command(aliases = {"report"}, desc = "Записывает отчёт о WorldGuard", flags = "p", max = 0)
    ...
    @Command(aliases = {"profile"}, usage = "[-p] [-i <интервал>] [-t <фильтр потока>] [<минуты>]",
            desc = "Профилирует использование ЦП сервера", min = 0, max = 1,
            flags = "t:i:p")
    ...
    public void profile(final CommandContext args, final Actor sender) throws CommandException, AuthorizationException {
        ...
        if (args.argsLength() == 0) {
            ...
            if (minutes < 1) {
                throw new CommandException("Вы должны запускать профилирование на не менее 1 минуты.");
            } else if (minutes > 10) {
        ...
        if (args.hasFlag('i')) {
            ...
            if (interval < 1 || interval > 100) {
                throw new CommandException("Интервал должен быть между 1 и 100 (в миллисекундах)");
            }
            if (interval < 10) {
        ...
        synchronized (this) {
            if (activeSampler != null) {
                throw new CommandException("Профилирование находится в разработке! Пожалуйста, используйте /wg stopprofile для отмены текущего профилирования.");
            }
        ...
        sender.print(TextComponent.of("Запуск профилирования ЦП. Результаты будут доступны через " + minutes + " минут.", TextColor.LIGHT_PURPLE)
                ...
                .append(TextComponent.of(" в любое время для отмены профилирования ЦП.", TextColor.GRAY)));

        worldGuard.getSupervisor().monitor(FutureForwardingTask.create(
                sampler.getFuture(), "Профилирование ЦП за " + minutes + " минут", sender));
    ...
    public void stopProfile(CommandContext args, final Actor sender) throws CommandException {
        synchronized (this) {
            if (activeSampler == null) {
                throw new CommandException("Профилирование ЦП сейчас не запущено.");
            }
        ...
        sender.print("Текущее профилирование ЦП было отменено.");
    }
    ...
    public void flushStates(CommandContext args, Actor sender) throws CommandException {
        if (args.argsLength() == 0) {
            ...
            if (player != null) {
                ...
                sender.print("Очищенные состояния для игрока \"" + player.getName() + "\".");
            }
        }
    }
    ...
    public void listRunningTasks(CommandContext args, Actor sender) throws CommandException {
        ...
        if (tasks.isEmpty()) {
            sender.print("Сейчас нет запущенных задач.");
        } else {
