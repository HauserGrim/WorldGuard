...
class FlagHelperBox extends PaginationBox {
    ...
    private void appendFlagName(TextComponent.Builder builder, Flag<?> flag, TextColor color) {
        ...
        if (flag == Flags.PASSTHROUGH) {
            builder.append(TextComponent.empty().append(TextComponent.of("*", TextColor.AQUA))
                    .hoverEvent(HoverEvent.of(HoverEvent.Action.SHOW_TEXT,
                            TextComponent.of("Это специальный флаг, который переопределяет проверки строительства."))));
            length += monoSpace ? 1 : FlagFontInfo.getPxLength('*');
        }
    ...
    private <V> void appendValueChoices(TextComponent.Builder builder, Flag<V> flag, Iterator<V> choices, @Nullable String suggestChoice) {
        ...
        while (choices.hasNext()) {
            ...
            if (maySet) {
                if (isExplicitSet) {
                    hoverTexts.add(TextComponent.of("Нажмите для удаления", TextColor.GOLD));
                    ...
                    hoverTexts.add(TextComponent.of("Установка этого флага может иметь непредсказуемые последствия.", TextColor.RED)
                            ...
                            .append(TextComponent.of("Пожалуйста, прочитайте документацию и установите этот флаг вручную, если Вы действительно намерены это сделать.")
                            .append(TextComponent.newline())
                            .append(TextComponent.of("(Совет: Вам не нужно устанавливать это для защиты региона!)"))));
                } else {
                    hoverTexts.add(TextComponent.of("Нажмите для установки", TextColor.GOLD));
                }
            }
        ...
        if (suggestChoice != null && perms.maySetFlag(region, flag)) {
            builder.append(TextComponent.of(suggestChoice, TextColor.DARK_GRAY)
                    .hoverEvent(HoverEvent.of(HoverEvent.Action.SHOW_TEXT,
                            TextComponent.of("Нажмите для установки своего значения", TextColor.GOLD)))
    ...
    private <V> Component getToolTipHint(V defVal, V currVal, boolean inherited) {
        ...
        if (inherited) {
            if (currVal == defVal) {
                valType = TextComponent.of("Унаследованный и ")
    ...
    private void appendLocationFlagValue(TextComponent.Builder builder, LocationFlag flag) {
        ...
        if (currVal == null) {
            ...
            if (defVal == null) {
                appendValueText(builder, flag, "координаты не заданы", null);
            } else {
    ...
    private <V extends Number> void appendNumericFlagValue(TextComponent.Builder builder, NumberFlag<V> flag) {
        ...
        appendValueChoices(builder, flag, (Iterator<V>) choices.iterator(), choices.isEmpty() ? "число не задано" : "[своё]");
    }
    ...
    private void appendStringFlagValue(TextComponent.Builder builder, StringFlag flag) {
        ...
        if (currVal == null) {
            ...
            if (defVal == null) {
                appendValueText(builder, flag, "текст не задан", null);
            } else {
