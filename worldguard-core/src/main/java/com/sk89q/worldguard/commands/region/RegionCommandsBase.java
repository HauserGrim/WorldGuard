...
class RegionCommandsBase {
    ...
    protected static ProtectedRegion checkExistingRegion(RegionManager regionManager, String id, boolean allowGlobal) throws CommandException {
        ...
        if (region == null) {
            ...
            throw new CommandException(
                    "Регион с названием '" + id + "' не найден.");
        }
    ...
    protected static ProtectedRegion checkRegionStandingIn(RegionManager regionManager, LocalPlayer player, boolean allowGlobal, String rgCmd) throws CommandException {
        ...
        if (set.size() == 0) {
            if (allowGlobal) {
                ...
                player.printDebug("Вы не стоите ни в одном регионе. " +
                        "Вместо этого используется глобальный регион этого мира.");
                return global;
            }
            throw new CommandException(
                    "Вы не стоите ни в одном регионе." +
                            "Укажите ID, если хотите выбрать какой-то конкретный регион.");
        } else if (set.size() > 1) {
            boolean first = true;
            ...
            for (ProtectedRegion region : set) {
                ...
                if (rgCmd != null && rgCmd.contains("%id%")) {
                    regionComp = regionComp.hoverEvent(HoverEvent.of(HoverEvent.Action.SHOW_TEXT, TextComponent.of("Нажмите для выбора этого региона")))
    ...
    protected static Region checkSelection(Actor actor) throws CommandException {
        ...
        try {
            ...
            throw new CommandException("Пожалуйста, сначала выделите область. " +
    ...
    protected static ProtectedRegion checkRegionFromSelection(Actor actor, String id) throws CommandException {
        ...
        if (selection instanceof Polygonal2DRegion) {
            ...
            throw new CommandException("К сожалению, только кубоиды и полигоны могут быть регионами в WorldGuard.");
        }
    }
    ...
    protected static void warnAboutSaveFailures(Actor sender) {
        ...
        if (!failures.isEmpty()) {
            ...
            sender.print(TextComponent.of("(Предупреждение: Не удалось сохранить данные региона в этих мирах: " + failingList + ". " +
                    "Ваши изменения потеряны. Смотрите журнал сервера для дополнительной информации.)", TextColor.GOLD));
        }
    }
        ...
        if (spawn != null) {
            if (!spawn.getIntersectingRegions(ImmutableList.of(region)).isEmpty()) {
                sender.print(ErrorFormat.wrap("Предупреждение!")
                        .append(TextComponent.of(" Эта область перекрывается ванильной защитой спауна. WorldGuard не может " +
