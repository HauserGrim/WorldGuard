...
public class SimpleDomainRegistry implements DomainRegistry {
    ...
    public void register(String name, DomainFactory<?> domain) throws DomainConflictException {
        synchronized (lock) {
            if (initialized) {
                throw new IllegalStateException("Новые домены не могут быть зарегистрированы в настоящее время");
            }
    ...
    private <T extends DomainFactory<?>> T forceRegister(String name, T domain) throws DomainConflictException {
        ...
        if (!CustomDomain.isValidName(name)) {
            throw new IllegalArgumentException("Использовано недопустимое название домена.");
        }

        synchronized (lock) {
            if (domains.containsKey(name)) {
                throw new DomainConflictException("Домен с названием " + name + " уже существует");
            }
    ...
    public List<CustomDomain> unmarshal(Map<String, Object> rawValues, boolean createUnknown) {
        ...
        for (Map.Entry<String, Object> entry : rawValues.entrySet()) {
            try {
                ...
                log.log(Level.WARNING, "Не удалось распаковать домен для " + entry.getKey(), e);
            }
        }
        return domainList;
    }
