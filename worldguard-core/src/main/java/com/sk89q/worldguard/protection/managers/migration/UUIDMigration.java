...
public class UUIDMigration extends AbstractMigration {
    ...
    private boolean migrate(Collection<ProtectedRegion> regions) throws MigrationException {
        ...
        if (!names.isEmpty()) {
            ...
            try {
                ...
                log.log(Level.INFO, "Преобразование " + names.size() + " имён в UUID... это может занять некоторое время.");
                ...
                throw new MigrationException("Ошибка службы имя -> UUID", e);
            ...
            log.log(Level.INFO, "UUID преобразованы... теперь миграция всех регионов на UUID, где это возможно...");
            convert(regions);
        ...
        if (!unresolvedNames.isEmpty()) {
            if (keepUnresolvedNames) {
                log.log(Level.WARNING,
                        "Некоторые имена участников и владельцев, похоже, не существуют или не владеют Minecraft, " +
                                "поэтому их нельзя преобразовать в UUID. Они были оставлены как имена, но преобразование " +
                                "можно запустить повторно, установив для параметра 'keep-names-that-lack-uuids' значение " +
                                "false в конфигурации, чтобы удалить эти имена. Сохранение имен означает, что кто-то может " +
                                "зарегистрироваться под одним из этих имен в будущем и стать этим игроком.");
            } else {
                log.log(Level.WARNING,
                        "Некоторые имена участников и владельцев, похоже, не существуют или не владеют " +
                                "Minecraft, поэтому их нельзя преобразовать в UUID. Эти имена были удалены.");
            }
        }
    }
    ...
    private class ResolvedNamesTimerTask extends TimerTask {
        @Override
        public void run() {
            log.info("UUID были найдены для " + resolvedNames.size() + " имён...");
        }
    }
